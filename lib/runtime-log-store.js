  ...
  function readRuntimeEvents() {
    ...
    try {
      for (let index = maxFiles - 1; index >= 1; index -= 1) {
        const filePath = path.join(logDir, rotatedName(index));
      ...
      if (fs.existsSync(activePath)) {
        filesRead += 1;
        for (const line of fs.readFileSync(activePath, "utf8").split(/\r?\n/)) {
          if (line.trim() === "") continue;
          try {
            events.push(JSON.parse(line));
          } catch {
      ...
      // Diagnostics must remain available when a log file is unreadable.
    }
    return {
      events,
      health: {
