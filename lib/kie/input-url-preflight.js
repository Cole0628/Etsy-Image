...
function mapKieFailureMessage(message) {
  ...
  if (/fetch failed|failed to fetch|networkerror|econnreset|enotfound|etimedout|econnrefused/i.test(text)) {
    return "无法连接 Kie 服务，请检查这台电脑的网络、VPN/代理、防火墙或杀毒软件。";
  }
  if (/unexpected token.*<!doctype|not valid json|返回了网页而不是 json/i.test(text)) {
    return "Kie 接口返回了网页而不是 JSON，通常是网络代理、登录页、防火墙或网关拦截导致。";
  }
